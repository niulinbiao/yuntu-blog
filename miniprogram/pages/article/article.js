...
Page({
    ...
    wx.request({
      ...
      success: function (res) {
        if(res.data.status == 200){
          ...
          console.log("authorinfo:"+JSON.stringify(res.data.data))
  ...
  loadArticleDetail(articleId){
    ...
    wx.request({
      ...
      success: function (res) {
        if(res.data.status == 200){
          console.log("文章数据："+JSON.stringify(res.data.data))
          //添加图片域名
          // res.data.data.thumbnail = app.globalData.domain+res.data.data.thumbnail
    ...
    if(app.globalData.userInfo && app.globalData != undefined){
      ...
      wx.getUserProfile({
        ...
        success: (res) => {
          ...
          this.setData({
            avatarUrl: app.globalData.domainn+res.userInfo.avatarUrl,
  ...
  loadComments(postId){
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        // console.log("用户评论："+JSON.stringify(res.data.data))
        if(res.data.status == 200){
          that.setData({
